...
public class Ex6BigData {

    // Use true to use hardcoded data, false to load data from files.
    private static final boolean IS_TESTING_MODE = false;
    private static final String DATA_FOLDER = "apache-spark-rdd-demo/src/main/resources/viewing-figures/";
    ...
    private static JavaPairRDD<Integer, String> setUpTitlesDataRdd(JavaSparkContext sc) {
        ...
        return sc.textFile(DATA_FOLDER + "titles.csv")
    ...
    private static JavaPairRDD<Integer, Integer> setUpChapterDataRdd(JavaSparkContext sc) {
        ...
        return sc.textFile(DATA_FOLDER + "chapters.csv")
                .mapToPair(commaSeparatedLine -> {
                    String[] cols = commaSeparatedLine.split(",");
    ...
    private static JavaPairRDD<Integer, Integer> setUpViewDataRdd(JavaSparkContext sc) {
        ...
        return sc.textFile(DATA_FOLDER + "views-*.csv")
                .mapToPair(commaSeparatedLine -> {
                    String[] cols = commaSeparatedLine.split(",");
                    return new Tuple2<>(Integer.parseInt(cols[0]), Integer.parseInt(cols[1]));
                });
